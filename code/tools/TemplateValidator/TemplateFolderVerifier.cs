...
namespace TemplateValidator
{
    ...
    {
        public static VerifierResult VerifyTemplateFolders(bool showWarnings, bool checkPrimaryOutput, IEnumerable<string> templateFolders)
        {
            ...
            if (!results.Any())
            {
                ...
                foreach (var templateFilePath in allTemplateFilePaths)
                {
                    ...
                    // SIB: 1. Check for duplicate identities
                    if (template.Identity != null)
                    {
                        ...
                        // SIB: 2. Check that localized files have the same identity
                    ...
                    foreach (var file in new DirectoryInfo(templateRoot).GetFiles("*.*", SearchOption.AllDirectories))
                    {
                        ...
                        {
                            // SIB: 3. Check primary output

                            // Projects will include files that aren't directly referenced
                            if (checkPrimaryOutput && template.GetTemplateOutputType() == TemplateOutputType.Item)
                            {
                            ...
                            if (showWarnings)
                            {
                                // SIB: 4. Check for duplicated files (based on file hash)
                ...
                // SIB: 5. Check dependencies exist
                ...
                // SIB: 6. Check requirements exist
                ...
                // SIB: 8. Check composition filters
